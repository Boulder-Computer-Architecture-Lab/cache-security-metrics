...
let speclist = [
    ...
    ("--brrip", Arg.Unit (fun () -> data_cache_strategy := CacheAD.BRRIP), 
      "set the cache replacement strategy to BRRIP");
    ("--fifo", Arg.Unit (fun () -> data_cache_strategy := CacheAD.FIFO), 
      "set the cache replacement strategy to FIFO");
    ("--bip", Arg.Unit (fun () -> data_cache_strategy := CacheAD.BIP), 
      "set the cache replacement strategy to BIP");
    ("--rand", Arg.Unit (fun () -> data_cache_strategy := CacheAD.RAND), 
      "set the cache replacement strategy to random");
    ("--rr", Arg.Unit (fun () -> data_cache_strategy := CacheAD.RR), 
      "set the cache replacement strategy to RR");
    ("--mru", Arg.Unit (fun () -> data_cache_strategy := CacheAD.MRU), 
      "set the cache replacement strategy to MRU");
    ...
    ("--inst-mru", Arg.Unit (fun () -> inst_cache_strategy_opt := Some CacheAD.MRU), 
      "set the instruction cache replacement strategy to MRU");
    ("--inst-rr", Arg.Unit (fun () -> inst_cache_strategy_opt := Some CacheAD.RR), 
      "set the instruction cache replacement strategy to RR");
    ("--inst-bip", Arg.Unit (fun () -> inst_cache_strategy_opt := Some CacheAD.BIP), 
      "set the instruction cache replacement strategy to BIP");
    ("--inst-brrip", Arg.Unit (fun () -> inst_cache_strategy_opt := Some CacheAD.BRRIP), 
      "set the instruction cache replacement strategy to BRRIP");
    ("--inst-fifo", Arg.Unit (fun () -> inst_cache_strategy_opt := Some CacheAD.FIFO), 
      "set the cache replacement strategy to FIFO");
    ("--inst-rand", Arg.Unit (fun () -> inst_cache_strategy_opt := Some CacheAD.RAND), 
      "set the cache replacement strategy to random");

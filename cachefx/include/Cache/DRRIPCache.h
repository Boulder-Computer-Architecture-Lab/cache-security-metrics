...
public:
  ...
  DRRIPCache(const std::string &description, replAlg alg, int32_t nCacheLines,
    ...
    if (description == SetAssocCache::CACHE_TYPESTR) {
      ...
      SRRIPCache =
          std::unique_ptr<Cache>(new AssocCache(REPL_SRRIP, nCacheLines));
      BRRIPCache =
          std::unique_ptr<Cache>(new AssocCache(REPL_BRRIP, nCacheLines));
      ...
      perror("Incompatible configuration: MIRAGE and DIP Replacement!\n");
      exit(-1);
     } else if (description == SassCache::CACHE_TYPESTR) {
      perror("Incompatible configuration: SassCache and DIP Replacement!\n");
      exit(-1);

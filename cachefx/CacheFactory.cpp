...
CacheFactory::CacheFactory() {
  ...
  strRepAlgMap["fifo"] = REPL_FIFO;
  strRepAlgMap["round-robin"] = REPL_RR;
  strRepAlgMap["mru"] = REPL_MRU;
}
...
std::unique_ptr<Cache> CacheFactory::createCache(const std::string &description,
  ...
  if (replAlgorithm != REPL_DIP) {
    ...
    } else if (description == SassCache::CACHE_TYPESTR) {
      return std::unique_ptr<Cache>(
          new SassCache(nCacheLines / nWays, nWays));
    } else if (description == SkewedAssocCache::CACHE_TYPESTR) {
      return std::unique_ptr<Cache>(
          new SkewedAssocCache(replAlgorithm, nCacheLines / nWays, nWays));
    ...
    } else if (description == MIRAGE::CACHE_TYPESTR) {
      return std::unique_ptr<Cache>(
          new MIRAGE(nCacheLines / nWays, nWays));
...
void CacheFactory::serializeCache(pugi::xml_node xmlCache, Cache *cache) {
  ...
  } else if (cacheType == MIRAGE::CACHE_TYPESTR) {
    MIRAGE *c = (MIRAGE*) cache;
    xmlCache.append_attribute(XMLNODE_CACHE_NLINES).set_value(c->getNSets()*c->getNWays());
    xmlCache.append_attribute(XMLNODE_CACHE_NWAYS).set_value(c->getNWays());
    xmlCache.append_attribute(XMLNODE_CACHE_NPARTITIONS).set_value(c->getNPartitions());
  } else if (cacheType == SkewedAssocCache::CACHE_TYPESTR) {
    SkewedAssocCache *c = (SkewedAssocCache *)cache;
    xmlCache.append_attribute(XMLNODE_CACHE_NLINES)
        .set_value(c->getNSets() * c->getNWays());
    xmlCache.append_attribute(XMLNODE_CACHE_NWAYS).set_value(c->getNWays());
    xmlCache.append_attribute(XMLNODE_CACHE_REPLALG)
...
std::unique_ptr<Cache> CacheFactory::deserializeCache(pugi::xml_node xmlCache) {
  ...
  if (strRepAlgMap[xmlCache.attribute(XMLNODE_CACHE_REPLALG).as_string()] ==
    ...
    if (cacheType == SetAssocCache::CACHE_TYPESTR) {
      ...
      int32_t nWays = xmlCache.attribute(XMLNODE_CACHE_NWAYS).as_int();
      replAlg alg = REPL_RANDOM;
      if (!xmlCache.attribute(XMLNODE_CACHE_REPLALG).empty()) {
        alg =
            strRepAlgMap[xmlCache.attribute(XMLNODE_CACHE_REPLALG).as_string()];
      }
      cache.reset(new SkewedAssocCache(
          alg, xmlCache.attribute(XMLNODE_CACHE_NLINES).as_int() / nWays,
          nWays));
    } else if (cacheType == SkewedAssocCache::CACHE_TYPESTR) {
      int32_t nWays = xmlCache.attribute(XMLNODE_CACHE_NWAYS).as_int();
      replAlg alg = REPL_RANDOM;

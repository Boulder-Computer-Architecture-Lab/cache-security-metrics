...
int32_t NewCache::readCl(tag_t cl, const CacheContext &context,
  ...
  if (blk && (blk->getTag() == tag)) {
    ...
    if (blk->getTag() == TAG_NONE) { // invalid line, directly insert
      blk->setTag(tag);
      ...
      // lnreg));
      lnregMap[key] = blk;
    } else { // erase old mapping and insert new mapping
      LnregKey old_key =
...
NewCacheLine *NewCache::lnregMapLookup(const CacheContext &context,
                                       tag_t lnreg) {
  LnregKey key = std::make_pair(context.getCoreId(), lnreg);

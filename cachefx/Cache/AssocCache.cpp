...
void AssocCache::access(int32_t way, bool newEntry) {
  // if newEntry is true, then we had an eviction
  const auto timestamp_mru = [&]() { entries[way].accessTime = clock++; };
  ...
  if (newEntry && algorithm == REPL_LIP) {
    ...
    if (mru_chance <= bip_throttle) {
      ...
      timestamp_lru();
  } else if (algorithm == REPL_FIFO) {
    timestamp_mru();
  } else if (newEntry && algorithm == REPL_RR) {
    // Only update on eviction
    rr = (rr + 1) % size;
  } else {
    timestamp_mru();
  }
...
int32_t AssocCache::readCl(tag_t cl, const CacheContext &context,
  ...
  for (int32_t i = 0; i < size; i++) {
    ...
    if (entries[i].tag == TAG_NONE || entries[i].tag == TAG_INIT)
      free = i;
  }
  ...
  case REPL_RR:
    ...
    break;
  case REPL_MRU: {
      uint32_t youngestmru = std::numeric_limits<uint32_t>::max();
  ...
  case REPL_FIFO:
    // Choose the entry with the oldest insert timestamp.
    uint32_t oldestfifo = std::numeric_limits<uint32_t>::max();
    for (int32_t i = 0; i < size; i++) {
      uint32_t fifo_time = entries[i].accessTime;
      if (fifo_time < oldestfifo) {
        oldestfifo = fifo_time;
        free = i;
      }
    }
    break;
  }
  if (entries[free].tag == TAG_NONE || entries[free].tag == TAG_INIT) {
    response.push_back(CacheResponse(false));
  } else {

...
void ConfigWindowView::setCachePropertyTable(
    QStandardItemModel *propertyTable) {
  qTreeProperties->setModel(propertyTable);
  qTreeProperties->resizeColumnToContents(0);
  qTreeProperties->resizeColumnToContents(1);
}

void ConfigWindowView::installReplAlgComboBox(
    int row, QStringListModel *replAlgList, ConfigWindowPresenter *presenter) {
  if (row != -1) {

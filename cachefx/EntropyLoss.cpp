...
EntropyLoss::EntropyLoss() {
  ...
  repAlgStrMap[REPL_FIFO] = "fifo";
  repAlgStrMap[REPL_RR] = "round-robin";
  repAlgStrMap[REPL_MRU] = "mru";
}
...
void EntropyLoss::replaceAndPromoteCacheLineFront(tag_t oldTag, tag_t newTag) {
  ...
  if (found && rank < _evictionCnt.size()) {
    _evictionCnt[rank]++;
  }
...
void EntropyLoss::accessAndTrackOrder(MemHandle *memHandle, address_t offset) {
  ...
  // Hierarchies populate response level, but direct caches often keep it
  // unknown. If no known level exists, consume all responses.
  bool hasKnownLevel = false;
  ...
  for (auto resp : responses) {
    if (!hasKnownLevel || resp._level != CACHE_LEVEL_UNKNOWN) {
      if (resp._hit) {
...
void EntropyLoss::accessAndTrackContent(MemHandle *memHandle,
                                        address_t offset) {
  std::list<CacheResponse> responses;
  tag_t tag = memHandle->translate(offset) / CACHE_LINE_SIZE;
  memHandle->read(offset, responses);
...
double EntropyLoss::measureInformationFlow(uint32_t numExperiments,
  ...
  // Fallback to deterministic sweeping if random priming did not fully populate
  // the tracked cache content.
  for (uint32_t pass = 0;
       _faCache.size() < mmu->getCache()->getNLines() && pass < 4; pass++) {
    for (int32_t l = 0;
         _faCache.size() < mmu->getCache()->getNLines() && l < numCacheLines;
         l++) {
      accessAndTrackContent(memHandle, l * CACHE_LINE_SIZE);
    }
  }
  ...
  if (!partitionedCache &&
      _faCache.size() != mmu->getCache()->getNLines()) {
    std::cerr << "WARNING: Incomplete cache prefill tracking for "
              << mmu->getCache()->getCacheType() << " (tracked "
              << _faCache.size() << "/" << mmu->getCache()->getNLines()
              << "). Continuing with observed state." << std::endl;
  }
  ...
  _lastUniSECResult = calcUniSEC(numCacheLines);
...
void EntropyLoss::evaluate() {
  ...
  for (auto cfg : _evaluationConfigs) {
    ...
    if (evaluationType == PATTERN_LINEAR) {
      ...
      _unisecResults.push_back(UniSECResult());
      ...
      _unisecResults.push_back(_lastUniSECResult);
    } else {
      _unisecResults.push_back(UniSECResult());
    }
  }
...
void EntropyLoss::printStatistics(std::string &outputFilePath) {
  ...
  outputFile << delimiter << "Entropy" << delimiter << "UnisecREE" << delimiter
             << "UnisecWCL" << delimiter << "UnisecPidx" << delimiter << "UnisecESS" << std::endl;
  ...
  for (size_t i = 0; i < _evaluationConfigs.size(); i++) {
    ...
    outputFile << delimiter << result;
    if (i < _unisecResults.size()) {
      auto unisec = _unisecResults[i];
      outputFile << delimiter << unisec.ree << delimiter << unisec.wcl
                 << delimiter << unisec.predictabilityIndex << delimiter << unisec.ess;
    }
    outputFile << std::endl;
  }
  outputFile.close();
...
UniSECResult EntropyLoss::calcUniSEC(size_t nTotal) {
  UniSECResult result = {};
  result.nTotal = nTotal;

  uint64_t totalEvictions =
      std::accumulate(_evictionCnt.begin(), _evictionCnt.end(), uint64_t(0));
  result.totalEvictions = totalEvictions;

  if (totalEvictions == 0 || nTotal == 0) {
    return result;
  }
  ...
  for (int32_t i = 0; i < _evictionCnt.size(); i++) {
    if (_evictionCnt[i] != 0) {
      double p = (double)_evictionCnt[i] / totalEvictions;
      result.shannonEntropy -= p * std::log2(p);
      if (p > maxProbability) {
        maxProbability = p;
      }
      result.aacls++;
  ...
  result.minEntropy = -std::log2(maxProbability);
  result.ree = std::log2((double)nTotal) - result.shannonEntropy;
  result.wcl = std::log2((double)nTotal) - result.minEntropy;

  if (result.shannonEntropy != 0) {
    result.predictabilityIndex =
        (result.shannonEntropy - result.minEntropy) / result.shannonEntropy;
  }

  result.ess =
      ((double)result.aacls * (1 - result.predictabilityIndex)) / nTotal;
